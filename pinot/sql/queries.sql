...
select count(1), sum(total_amount), city from enriched_orders where request_time > toDateTime(now() - 60 * 60 * 1000, 'yyyy-MM-dd HH:mm:ss Z')  group by city order by 1 desc
